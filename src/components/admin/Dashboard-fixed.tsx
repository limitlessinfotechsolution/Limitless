...
const Dashboard: React.FC = () => {
  ...
  const [projectTimelineData, setProjectTimelineData] = useState<ChartDataPoint[]>([]);
  const [approvalData, setApprovalData] = useState<ChartDataPoint[]>([]);
  const [projectStatusData, setProjectStatusData] = useState<ChartDataPoint[]>([]);
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                cy="50%"
                labelLine={false}
                label={(entry: ChartDataPoint) => `${entry.name} ${((entry.value / projectStatusData.reduce((sum, item) => sum + item.value, 0)) * 100).toFixed(0)}%`}
                outerRadius={80}
                fill="#8884d8"
                dataKey="value"
              >
